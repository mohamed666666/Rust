...
fn main() {
    let mut t2shutdown:Rc<bool>=Rc::new(false);
    ...
    println!("connection recieved on : {:?}",socket);
     ...
     std::thread::spawn(|| {//spawn dunction return refrance called johnhandle object used as remote controll over the thread
         while !*t2shutdown{//
